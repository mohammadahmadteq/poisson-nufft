...
\begin{equation}
    ...
    \label{eq:coulomb_ft}
\end{equation}
This transforms the six-dimensional integral in \eqref{eq:eri} into a product of three-dimensional Fourier transforms, reducing it by the convolution theorem to:
\begin{equation}
...
The goal of this study is to benchmark several numerical approaches for computing ERIs, listed here: on Cartesian and Becke grids in real space, Poisson solver on a Cartesian grid, with Cartesian and spherical $k$-space grids, finally with Cartesian and spherical $k$-space grids.
We evaluate convergence behaviour, computational cost, and memory usage, and then will analyze based on the results the mathematical reasons behind the observed performance differences.
...
Hence, the direct methods, while conceptually simple, suffer from two compounding issues. The $\mathcal{O}(N^2)$ pairwise summation is inherently expensive, and the discretisation of the $1/r$ singularity introduces systematic errors that diminish slowly with grid refinement. The self-interaction exclusion (setting the distance matrix diagonal to infinity) is a necessary but imperfect remedy.
...
In order to reduce complex electron repulsion integrals with time complexity reaching upto $\mathcal{O}(N^2)$ we have to utilize numerical methods to calculate these integrals. Overall, a construction of these integrals is first done analytically using GTO orbitals in PySCF. In order to calculate these integrals numerically, we have to discretize the integral using quadrature rules. We have used three types of grids: Cartesian, Becke and spherical. The Cartesian grid is a simple uniform grid that covers a cubic domain, while the Becke grid is an atom-centred grid that concentrates points near the nuclei, and the spherical grid is a non-uniform grid in $k$-space that exploits the isotropy of the Coulomb kernel. Methods utilizing fourier transforms (FFT and NUFFT) are used to compute the ERIs in $k$-space, while direct summation methods are used in real space. The two-step Poisson--NUFFT method is a hybrid approach that first computes an intermediate potential in $k$-space and then transforms back to real space for final integration. Two-step poisson method with spherical $k$-grid achieves the best convergence and accuracy due to its ability to handle the Coulomb singularity and exploit the isotropy of the kernel. The one-step NUFFT method with spherical $k$-grid also performs well, but is limited by the single transform approximation. Direct methods and uniform FFT are less accurate and less efficient due to their inability to adapt to the molecular geometry and handle singularities effectively. Results of the experiment show that both NUFFT techniques show used superior convergence and accuracy compared to direct methods and uniform FFT. However, two-step and one-step methods show similar convergence rates, but the two-step method benefits from error cancellation and better conditioning due to the intermediate potential representation.
